...
export default function FilterableItemList({
  ...
  const active = model.length > 0 && itemList.length > 0
  ...
  return (
    <div className="filterable-item-list-height w-1/3 p-2 pl-4 bg-gray-50">
      ...
      <button onClick={onAddItem} disabled={!active} title={!active ? 'Загрузите модель и объекты' : ''} className={`my-2 py-1 px-4 ${active ? 'bg-gray-200' : 'bg-gray-100'} `}>+</button>
      <ItemList
        items={itemList}
